...
class Downloader(object):
    ...
    Private class members:
        ...
        build_dir: the local filesystem directory to store artifacts for the
            given build based on the remote source.

    Public methods must be overridden:
        Wait: Verifies the local artifact exists and returns the appropriate
            names.
    ...
    def _TryRemoveStageDir(directory_path):
        ...
        staged.timestamp is the only file in that directory. The build could be
        non-existing, and the directory should be removed.
    ...
    def ListBuildDir(self):
        ...
        if not os.path.isdir(self._build_dir):
            raise build_artifact.ArtifactDownloadError(
                "Artifacts %s improperly staged to build_dir path %s. The path "
                "is not a directory." % (self._archive_url, self._build_dir)
            )
    ...
    def Download(self, factory):
        ...
        the devserver. A call to this will attempt to cache non-specified
        artifacts in the background following the principle of spatial locality.

        Args:
            factory: The artifact factory.

        Raises:
            build_artifact.ArtifactDownloadError: If failed to download the
                artifact.
        """
        common_util.MkDirP(self._build_dir)

        # We are doing some work on this build -- let's touch it to indicate
        # that we shouldn't be cleaning it up anytime soon.
    ...
    def IsStaged(self, factory):
        ...
        Args:
            factory: An instance of BaseArtifactFactory to be used to check if
                desired artifacts or files are staged.

        Returns:
            True if all artifacts are staged.

        Raises:
            DownloaderException: A wrapper for exceptions raised by any artifact
    ...
    def _DownloadArtifactsInBackground(self, artifacts):
        ...
        Args:
            artifacts: List of build_artifact.BuildArtifact instances to
                download.
        """
        logging.debug(
...
class GoogleStorageDownloader(Downloader):
    ...
    def GetBuildIdFromArchiveURL(archive_url):
        ...
        This function discards 'gs://server/' and extracts the
        [some_path/target] as rel_path and the build as build.
        """
...
class LocalDownloader(Downloader):
    ...
    def __init__(self, static_dir, source_path, delete_source=False):
        ...
        Args:
            ...
            delete_source: If True, delete the source files. This mode is faster
                than actually copying because it allows us to simply move the
                files.
        """
        # The local path is of the form:
